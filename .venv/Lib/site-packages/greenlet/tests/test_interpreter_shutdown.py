...
guards protect against this on ALL Python versions:
  ...
  1. g_greenlet_shutting_down â€” set by an atexit handler registered at
  ...
  2. Py_IsFinalizing() â€” covers the GC collection and later phases of
...
class TestInterpreterShutdown(TestCase): # pylint:disable=too-many-public-methods
    ...
    # -----------------------------------------------------------------
    # Group A: Core safety â€” no crashes with active greenlets at exit
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # Group B: Cleanup semantics â€” thread exit
    # -----------------------------------------------------------------
    #
    ...
    # during atexit â€” registered AFTER greenlet import
    #
    ...
    def test_greenlet_construction_with_cross_thread_deleteme_during_atexit(self):
        ...
        thread, then drop them â€” populating the deleteme list. Then
    ...
    # -----------------------------------------------------------------
    # Group D.1: TDD-certified â€” getcurrent() during GC finalization
    #
    ...
    #   Python 3.10: RED (UNGUARDED) â†’ GREEN (GUARDED)
    #   Python 3.11: RED (UNGUARDED) â†’ GREEN (GUARDED)
    #   Python 3.12: RED (UNGUARDED) â†’ GREEN (GUARDED)
    #   Python 3.13: RED (UNGUARDED) â†’ GREEN (GUARDED)
    #   Python 3.14: RED (UNGUARDED) â†’ GREEN (GUARDED)
    # -----------------------------------------------------------------
    ...
    def test_getcurrent_returns_none_during_gc_finalization(self):
        ...
        this would return a greenlet â€” the same unguarded code path
    ...
    # -----------------------------------------------------------------
    # Group D.2: TDD-certified â€” getcurrent() during atexit phase
    #
    ...
    # the checker runs SECOND and observes getcurrent() â†’ None.
    #
    # This covers the atexit phase where _Py_IsFinalizing() is still
    # False on ALL Python versions â€” the exact window that causes
    # SIGSEGV in production (uWSGI worker recycling â†’ Py_FinalizeEx).
    #
    # TDD verification (greenlet 3.3.2 = RED, patched = GREEN):
    #   Python 3.10: RED (UNGUARDED) â†’ GREEN (GUARDED)
    #   Python 3.11: RED (UNGUARDED) â†’ GREEN (GUARDED)
